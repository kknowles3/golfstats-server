...
from components.loaders import updater
from dev.util.app_util import convert_utc_to_timezone
from components.loaders import EVENT_TAG, LOGO_URL
...
def create_page_header():
    ...
    page_header = html.H1('{} - Live Update Server'.format(EVENT_TAG))
...
def create_description():
    ...
    description = dbc.Alert([
        html.H4("{} - Live Update Server".format(EVENT_TAG), className='alert-heading'),
        dcc.Markdown(
            """This page is the live update server for the {}, with the ability to
...
def update_scores(n, recent_update_list):
    ...
    recent_updates = updater.mds.load_remote_data_items(collection_name, find_kwargs=find_kwargs)
