...
def load_csv_as_df(path, fname, index_col=False):
    """
    Loads a csv file into a dataframe.  Includes support for loading a csv
    file stored within a zip file (i.e., '.zip' filename extension) of the same
    base filename.  Does not currently handle the case where multiple csv files
    are stored within a single zip file.
...
def save_df_to_csv(df, path, fname, index_col=False):
    """
    Saves a dataframe to a local csv file.  This will automatically compress
    the saved fill when the fname extension if '.zip'.
...
def expand_dict_column(df, col_name):
    ...
    if col_name in df.columns.values:
        df = df.drop(col_name, axis=1).join(pd.DataFrame(df[col_name].values.tolist()))
